...
RUN touch $JAVA_HOME/conf/security/java.security && \
    chown 1000:0 $JAVA_HOME/conf/security/java.security && \
    chmod g+rw $JAVA_HOME/conf/security/java.security && \
    sed -i '/networkaddress.cache.ttl/d' $JAVA_HOME/conf/security/java.security && \
    sed -i '/networkaddress.cache.negative.ttl/d' $JAVA_HOME/conf/security/java.security && \
    echo 'networkaddress.cache.ttl=0' >> $JAVA_HOME/conf/security/java.security && \
    echo 'networkaddress.cache.negative.ttl=0' >> $JAVA_HOME/conf/security/java.security
